...
## **! NOT for the Public Explainer !**


## Alternative approach

There is currently no single, unified permission API for the web. The reason for this stems from the longstanding position that users should be prompted to grant permissions for each API both individually and only at the time the API would be used. As such, establishing a common time-limiting approach is difficult. That said, it should be achievable through the use of an optional configuration object:
...
This would require modifying a whole host of APIs:

### Geolocation

To request the location of a user, an author must write something like this:
...
To accommodate timeboxing, a `duration` key could be added to the `PositionOptions` object:
...
The `getUserMedia()` method already supports a [constraints object](https://w3c.github.io/mediacapture-main/#dom-mediastreamconstraints), which could be augmented to include a duration property:
...
Clipboard access is gated by four methods: `read()`, `readText()`, `write()`, and `writeText()`. These could be modified to take a configuration object:
...
Many sensitive APIs are being rolled into the [Generic Sensor API](https://w3c.github.io/sensors/), including the future `geolocation` successor `GeolocationSensor`. These support a [`SensorOptions`](https://w3c.github.io/sensors/#dictdef-sensoroptions) configuration which could be updated to support `duration`:
