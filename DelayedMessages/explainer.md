...
The following example codes demonstrates this situation, where where multiple `deleteMail` tasks congest the message queue before an urgent `readMail` task.
...
const sendOpenIndexedDBMessage = () => {
  // Read emails from IndexedDB
  worker.postMessage({ taskType: "readMail", startTime: performance.now() });
  console.log("[main] dispatching the readMail task");
};
...
While delays in background tasks like `deleteMail` might be acceptable, delays in user-initiated, high-priority tasks like `readMail` severely impact user experience. It's crucial for developers to identify if a browser context or worker is congested and which tasks contribute most to this congestion.
...
In the main.js file, 7000 JSON objects are sent to the worker using the `postMessage` API. The duration of serialization can be measured by calling the performance.now() API before and after executing `postMessage`. 
...
As shown, serialization on the main thread (approx. 130.30 ms) occurs synchronously during the `postMessage()` call, blocking other main thread work. Similarly, deserialization on the worker thread (approx. 114.30 ms) blocks the `onmessage` handler from processing other messages. The "message queue wait time + etc" reflects the time from `postMessage` initiation until the worker's handler begins, which is dominated here by the sender's serialization time.

Current performance APIs make it difficult to precisely measure serialization and deserialization times independently. Using `performance.now()` around `postMessage` on the sender side includes both serialization and the internal dispatching overhead. On the receiver side, measuring deserialization is also indirect. This API proposes to expose these timings directly.
...
### `postMessage` Timestamps and Durations:
  ...
  * **`sentTime`**: The timestamp when the message is successfully added to the task queue of the receiving browser context or worker.
...
This interface is available in both Windows and Web Workers, providing insights into `postMessage` timings. For round-trip timing analysis, developers would need to correlate entries from both the sender and receiver contexts (e.g., by sending performance entries back or using a unique identifier). Performance entry copies across origins will be restricted based on standard security policies.
...
Returns a `DOMHighResTimeStamp` representing the time when the `postMessage()` API was executed in the sending context. The timestamp is relative to the `timeOrigin` of the context where the `PerformanceDelayMessageTiming` entry is observed.
